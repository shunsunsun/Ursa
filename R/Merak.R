...
FlowPip <- function(project_name = "Ursa_Flow",
  ...
  for(j in 1:length(channels)){
    ...
    # somePNGPath <- paste(cdir,"1URSA_FLOW_ASINH_DENSITY_PLOT_CHANNEL_",channels[[j]],"_",project_name,".png", sep = "")
  ...
  data_summary <- NULL
  k <- 1
  somePDFPath <- paste(cdir,"2URSA_FLOW_GATING_STRATEGY_PART1_",project_name,".pdf", sep = "")
  pdf(file=somePDFPath, width=10, height=7.5,pointsize=12)
  for(i in 1:length(fs_data)){
    ...
    if(length(colnames(fs_data[[i]])[grep("FSC.*A|FSC.*H", colnames(fs_data[[i]]), ignore.case = T)]) == 2){
      ...
      names(p2plots)[k] <- paste(cname,lastGate, sep = ":")
      # somePNGPath <- paste(cdir,"2URSA_FLOW_GATING_STRATEGY_PART2_",chnlx,"_VS_",chnly,"_",cname,".png", sep = "")
      # png(somePNGPath, width = 4000, height =3000, units = "px", res = 300)
      print(p2plots[[k]])
      # dev.off()
      k <- k + 1
    ...
    if(length(colnames(fs_data[[i]])[grep("SSC.*W|SSC.*H", colnames(fs_data[[i]]), ignore.case = T)]) == 2){
      ...
      names(p2plots)[k] <- paste(cname,lastGate, sep = ":")
      # somePNGPath <- paste(cdir,"2URSA_FLOW_GATING_STRATEGY_PART3_",chnlx,"_VS_",chnly,"_",cname,".png", sep = "")
      # png(somePNGPath, width = 4000, height =3000, units = "px", res = 300)
      print(p2plots[[k]])
      # dev.off()
      k <- k + 1
    ...
    if(length(colnames(fs_data[[i]])[grep("FSC.*H|FSC.*W", colnames(fs_data[[i]]), ignore.case = T)]) == 2){
      ...
      names(p2plots)[k] <- paste(cname,lastGate, sep = ":")
      # somePNGPath <- paste(cdir,"2URSA_FLOW_GATING_STRATEGY_PART4_",chnlx,"_VS_",chnly,"_",cname,".png", sep = "")
      # png(somePNGPath, width = 4000, height =3000, units = "px", res = 300)
      print(p2plots[[k]])
      # dev.off()
      k <- k + 1
...
dev.off()
  ...
  somePNGPath = paste(zip_out,"12URSA_FLOW_UMAP_CLUSTERS_BY_SAMPLES_",project_name,".png", sep = "")
  ...
  # pdf(file=somePDFPath, width=10, height=7.5,pointsize=12)
  somePNGPath = paste(zip_out,"13URSA_FLOW_PCA_CLUSTERS_",project_name,".png", sep = "")
  png(somePNGPath, width = 4000, height = 3000,units = "px", res = 300)
  print(p13plots)
  dev.off()
  ...
  somePNGPath = paste(zip_out,"15URSA_FLOW_UMAP_MARKER_EXPRESSIONS_",project_name,".png", sep = "")
